...
function filterFiles(status) {
    const rows = document.querySelectorAll("#filesTable tr[data-status]");
...
// View file details (reuse same logic from dashboard.js)
...
// Display file details (same as dashboard)
          ...
          ${documents
            ...
            <div style="padding: 12px; background: #F5F5F5; border-radius: 8px; display: flex; justify-content: space-between; align-items: center;">
              ...
              <a href="/documents/${doc.file_path
                  .split("/")
                  .pop()}" target="_blank" class="btn btn--sm btn--outline">Télécharger</a>
            </div>
          `
              )
              .join("")}
...
async function approveFile(id) {
    if (!confirm("Êtes-vous sûr de vouloir approuver ce dossier?")) return;
